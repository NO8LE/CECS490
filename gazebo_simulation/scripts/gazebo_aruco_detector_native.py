...
"""
Native Gazebo ArUco Marker Detector for Precision Landing Simulation (No ROS)

This script detects ArUco markers in camera images from a native Gazebo simulation.
It is designed for autonomous precision landing simulations on Ubuntu without ROS:
...
Usage:
  python3 gazebo_aruco_detector_native.py [options]

Options:
  --target, -t MARKER_ID     Marker ID to use as landing target (default: 5)
  --camera-topic TOPIC       Gazebo camera topic (default: /drone/camera/link/camera/image)
...
# Import Gazebo interface helper
try:
    from gazebo_interface import GazeboInterface
    USE_GAZEBO_INTERFACE = True
except ImportError:
    USE_GAZEBO_INTERFACE = False
    print("Gazebo interface not found, using direct socket connection")
...
class ArUcoDetector:
    """ArUco marker detector for native Gazebo simulation"""
    ...
    def __init__(self, args):
        ...
        # Gazebo connection
        if USE_GAZEBO_INTERFACE:
            # Use Gazebo interface helper
            self.gazebo_interface = GazeboInterface()
            if self.gazebo_interface.connect():
                print("Connected to Gazebo using interface helper")
                self.gazebo_interface.subscribe_camera(args.camera_topic)
                self.gazebo = self.gazebo_interface  # For compatibility with get_latest_image()
            else:
                print("Failed to connect using interface helper, falling back to direct connection")
                self.gazebo = GazeboDirectConnection(args.camera_topic)
        else:
    ...
    def _detection_loop(self):
        ...
        while self.running:
            ...
            if USE_GAZEBO_INTERFACE and hasattr(self, 'gazebo_interface'):
                frame = self.gazebo_interface.get_camera_image(self.args.camera_topic)
            else:
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Native Gazebo ArUco Marker Detector')
